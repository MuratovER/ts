...
- Скачать скрипт() для объфявления локальных переменных с Notion->Work-in-progress->Разработка

### Объявление переменных окружения
Файл скачанный с Notion закидывается в корневую папку проекта ts
После чего производиться запуск
...
```
#### Windows
Запуск двойным нажатием на файл или же 
```
 cd c:\ts\setenv
